...
int n_threads = 10, max_d = 50, max_iter = 5000;
...
bool Player::over(bool islord, int limit_card_num) {
	if (islord) {
		return this->_handcards.empty();
	}
	if (this->_handcards.size() <= limit_card_num) {
		return true;
	}
	return false;
}
...
CardGroup Player::find_card_group(vector<Card> actionCards) {
	for (const auto &action : _avail_actions) {
		auto cards = action._cards;
		sort(cards.begin(), cards.end());
		if (cards.size() != actionCards.size()) {
			continue;
		}
		bool eq = true;
		for (int i = 0; i < cards.size(); i++) {
			if (cards[i] != actionCards[i]) {
				eq = false;
			}
		}
		if (eq) {
...
vector<CardGroup> Player::candidate_1(const CardGroup &last_card) {
	vector<CardGroup> its;
...
void MCPlayer::multisearch(vector<int> &cnts, MCState *root) {
	MCTree tree(root, vector<float>(), sqrtf(2.f));
...
CardGroup MCPlayer::respond(const CardGroup &last_card) {
	MCState *s = new MCState(*this->_env);
	auto action_space = s->get_action_space();
	vector<int> total_cnts(action_space.size(), 0);

	
	// random shuffle other player's card, determinized UCT
	int idx1 = (_env->_current_idx + 1) % _env->_players.size();

	vector<Card> unseen_cards = _env->_players[idx1]->_handcards;
	unseen_cards.insert(unseen_cards.end(), _env->_extra_cards.begin(), _env->_extra_cards.end());

	sort(unseen_cards.begin(), unseen_cards.end());
	cout << "id:" << _env->_current_idx <<"_env->_current_idx->_handcards=:" << s->_players[_env->_current_idx]->to_str() << endl;
	cout << "unseen_cards=";
	...
	cout << endl;
	//cout <<"unseen_cards=:" << unseen_cards << endl;
//	cout << last_card << endl;
...
//	cout << "action_space.size=" << action_space.size() << endl;
	if (action_space.size() > 1) 
	{
		...
		for (size_t d = 0; d < max_d; d++)
		{
			...
			//cout << "id:" << idx1 << "ss->_players[idx1]->_handcards=:" << ss->_players[idx1]->to_str() << endl;
			...
			MCTree tree(ss, vector<float>(), sqrtf(2.f));
			//tree.search(n_threads, max_iter);
			tree.search(5, 250);
			vector<int> cnts = tree.predict();
			for (size_t i = 0; i < action_space.size(); i++)
	...
	CardGroup cand = *action_space[max_element(total_cnts.begin(), total_cnts.end()) - total_cnts.begin()];
	cout << "³öÅÆ" << cand << endl;
	remove_cards(cand._cards);
	delete s;
	return cand;
