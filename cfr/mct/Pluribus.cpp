...
vector<mt19937> pluribusgenerators;

Pluribus::Pluribus(int numPlayers,int limitnum) {
	mNumPlayers = numPlayers;
	limitNum = limitnum;
	std::random_device mRd;
	std::mt19937 mActionEng(mRd());
...
void Pluribus::search(int n_threads, int n) {
	if (pluribusgenerators.empty()) {
		for (int i = 0; i < n_threads; i++) {
			pluribusgenerators.push_back(mt19937(random_device{}()));
		}
	}
	counter = n;
	...
	for (int i = 0; i < n_threads; i++) {
		...
		threads.push_back(std::move(std::thread(&Pluribus::search_thread, this, &pluribusgenerators[i])));
	}
	for (auto& t : threads) {
		t.join();
	}
...
void Pluribus::search_thread(mt19937 *generator) {
	while (true) {
		{
...
void Pluribus::subtrain() {
	for (int player = 0; player<mNumPlayers; player++) {
		/*
		if (i % mStrategyInterval == 0) {
		Pluribus::updateStrategy(mCurrentState, player);
		}*/
		if (counter > mPruneThreshold) {
			float prune = (float)rand() / RAND_MAX;
			if (prune < .05) {
				Pluribus::mccfr(mCurrentState, player, true);//Pluribus::mccfr(mCurrentState, player);
			}
			else {
...
std::valarray<float> Pluribus::mccfr(State state, int player, bool prune) {
	...
	if (currentPlayer == player) {
		...
		if (prune) {
			...
			for (auto action : validActions) {
				if (mNodeMap[currentPlayer].at(infoSet).regretSum.at(action) > mRegretMinimum) {
					returned = mccfr(State(state, action, limitNum), player, prune);
					utilities[action] = returned[currentPlayer];
					nodeUtil += returned * (float)strategy.at(action);
					explored.insert(action);
				}
		...
		std::discrete_distribution<int> random_choice(probabilities.begin(), probabilities.end());
		auto action = random_choice(mActionEng);
		return mccfr(State(state, actions[action], limitNum), player, prune);
	}
};
*/
std::valarray<float> Pluribus::mccfr(State& state, int player, bool prune) {
	...
	if (currentPlayer == player) {
		...
		if (prune) {
			...
			for (auto action : validActions) {
				if (mNodeMap[currentPlayer].at(infoSet).regretSum.at(action) > mRegretMinimum) {
					State *sstate = new State(state, action, limitNum);
					returned = mccfr(*sstate, player, prune);
					utilities[action] = returned[currentPlayer];
					nodeUtil += returned * (float)strategy.at(action);
					explored.insert(action);
					delete sstate;
		...
		std::discrete_distribution<int> random_choice(probabilities.begin(), probabilities.end());
		auto action = random_choice(mActionEng);
		State *sstate = new State(state, actions[action], limitNum);
		returned = mccfr(*sstate, player, prune);
		delete sstate;
		return returned;
	}
...
void Pluribus::updateStrategy(State state, int player) {
	...
	if (currentPlayer == player) {
		...
		mNodeMap[currentPlayer].at(infoSet).strategySum.at(actions[action]) += 1;
		updateStrategy(State(state, actions[action], limitNum), player);
	}
	else {
		std::string infoSet = state.infoSet();
		std::string actionSet = state.actionSet();
		...
		std::set<std::string> validActions = mValidActionsMap[actionSet];
		for (auto action : validActions) {
			updateStrategy(State(state, action, limitNum), player);
		}
	}
};
...
std::valarray<float> Pluribus::traverseTree(State state) {
	...
	for (auto action : validActions) {
		expectedUtility += traverseTree(State(state, action, limitNum)) * (float)strategy.at(action);
	}
	...
	if (currentPlayer == player) {
		...
		if (frozenAction == mFrozenNodes.end()) {
			...
			if (prune) {
				...
				for (auto action : validActions) {
					if (mNodeMap[currentPlayer].at(infoSet).regretSum.at(action) > mRegretMinimum) {
						returned = subgameSolve(State(state, action, limitNum), player, prune);
						utilities[action] = returned[currentPlayer];
						//nodeUtil += returned * strategy.at(action);
						nodeUtil += returned * (float)strategy.at(action);
						explored.insert(action);
			...
			// may need to check that we are not at a leaf node
			return subgameSolve(State(state, frozenAction->second, limitNum), player, prune);
		}
		...
		if (frozenAction == mFrozenNodes.end()) {
			...
			std::discrete_distribution<int> random_choice(probabilities.begin(), probabilities.end());
			auto action = random_choice(mActionEng);
			return subgameSolve(State(state, actions[action], limitNum), player, prune);
		}
		else {
			return subgameSolve(State(state, frozenAction->second, limitNum), player, prune);
		}
...
void Pluribus::subgameUpdate(State state, int player) {
	...
	if (currentPlayer == player) {
		...
		if (frozenAction == mFrozenNodes.end()) {
			...
			mNodeMap[currentPlayer].at(infoSet).strategySum.at(actions[action]) += 1;
			subgameUpdate(State(state, actions[action], limitNum), player);
		}
		else {
			subgameUpdate(State(state, frozenAction->second, limitNum), player);
		}
	}
	else {
		std::string infoSet = state.infoSet();
		...
		if (frozenAction == mFrozenNodes.end()) {
			...
			std::set<std::string> validActions = mValidActionsMap[actionSet];
			for (auto action : validActions) {
				subgameUpdate(State(state, action, limitNum), player);
			}
		}
		else {
			subgameUpdate(State(state, frozenAction->second, limitNum), player);
...
void Pluribus::opponentTurn(std::string action) {
	...
	mCurrentState = State(mCurrentState, action, limitNum);
};
...
void Pluribus::pluribusTurn() {
	...
	mFrozenNodes[infoSet] = actions[action];
	mCurrentState = State(mCurrentState, actions[action], limitNum);
};