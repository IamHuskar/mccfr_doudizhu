...
class OutcomeSamplingMCCFR:
    def __init__(self, root):
        ...
        self._expl = 0.6
        self._is_only_pass=False

    def iteration(self):
        self._episode(self.root, 0, 1, 1, 1, 0)
        #self._episode(self.root, 1, 1, 1, 1)

    def is_one_pass_state(self):
        return self._is_only_pass
    ...
    def _episode(self, state, update_player, my_reach, opp_reach, sample_reach, depth):
        if state.is_terminal():
            return 1./(depth+1) if state.player == update_player else -1./(depth+1)

        if state.is_only_one_state():
            self._is_only_pass=True
            return 0
        
        if state.is_chance_node():
            new_state = state.play()
            return self._episode(new_state, update_player, my_reach, opp_reach, sample_reach, depth)
        ...
        child_value = self._episode(new_state, update_player, new_my_reach,
                                    new_opp_reach, new_sample_reach, depth+1)
        child_values = {}
